...
class MainState extends State<Main> {
  ...
  Widget build(BuildContext context) {
    ...
    List<String> unit=[number.harmful+" ppm",number.dust+" µg/m3",number.temperature+" °C",number.humidity+" %"];
    ...
    Future<void> ShowNoti()async {
      ...
      else if (number.dust.toDouble() > overDust && Notiflag == false) {
        await showNotification();
        Notiflag = true;
      }
    }
    final Stream<int> _stream = (() {
      ...
      controller = StreamController<int>(
        onListen: () async {
             ...
             setState(() {
                unit=[number.harmful+" ppm",number.dust+" µg/m3",number.temperature+" °C",number.humidity+" %"];
              });
